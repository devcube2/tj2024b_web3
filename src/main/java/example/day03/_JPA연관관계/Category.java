...
public class Category {
    ...
    // + 양방향 , 게시물 여러개 참조
    // @OneToMany (mappedBy = "category") // 양방향 참조테이블은 자바에서만 참조한다.
...
/*
    - 영속성 제약조건 옵션 ( 1.cascade 2.fetch )
        [1] @OneToMany (cascade = CascadeType.XXX)
            ...
            3. cascade = CascadeType.MERGE   : 부모[PK] 가 삭제 되면 자식[FK]도 같이 수정됨. MERGE
            4. cascade = CascadeType.DETACH  : 부모[PK] 가 영속성 해지하면 자식[FK]도 같이 해제됨. DETACH
            5. cascade = CascadeType.PERSIST : 부모[PK] 가 저장하면 자식[FK]도 같이 저장됨. PERSIST
            6. cascade = CascadeType.REFRESH : 부모[PK] 가 새로 불러올때 자식[FK]도 같이 새로 불러온다. (새로고침)
        [2] @OneToMany (fetch = FetchType.XXX)
            1. fetch = FetchType.EAGER : 즉시 로딩(기본값) : 해당 엔티티를 조회할때 참조되는 모든 객체를 즉시 불러온다.
                특징 : 초기 로딩 느리다. 불필요한 엔티티가 모두 가져오기 때문에 (메모리)로드 기능 저하
            2. fetch = FetchType.LAZY  : 지연 로딩 : 해당 엔티티를 조회할때 참조되는 객체를 불러오지 않고 .getXXX() 등 참조할때 참조되는 객체를 불러온다.
